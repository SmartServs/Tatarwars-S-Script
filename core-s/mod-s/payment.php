...
class PaymentModel extends ModelBase
{
	...
	public function InsertMoneyLog( $transID, $usernam, $goldNumber, $cost, $currency, $type ){
        ...
        $this->provider->executeQuery("INSERT INTO user_payment (transID, usernam, golds, money, currency,type,time) VALUES ('$transID','$usernam', $goldNumber,$cost,'$currency','$type',".time().");");
	}
	
	public function updatetotalsms( $goldNumber, $cost )
    {
        $this->provider->executeQuery( "UPDATE money_total  SET total_gold=total_gold+%s, total_sms=total_sms+%s", array( $goldNumber, $cost ) );
    }
