...
$freegold=($AppConfig['Game']['freegold'])+35;

        return $this->provider->fetchResultSet( "SELECT * FROM `p_players` WHERE gold_num  > %s and total_people_count > 3 ORDER BY gold_num DESC, id ASC LIMIT %s,%s", array( $freegold, $pageIndexMeber * $pageSizeMeber, $pageSizeMeber ) );
    }
