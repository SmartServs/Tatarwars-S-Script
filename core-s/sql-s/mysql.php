...
class MysqlProvider
{
    ...
    public function open( )
    {
        ...
        $connKey = md5( $this->properties['webhost'].$this->properties['webuser'].$this->properties['webpassword'].$this->properties['webdatabase'] );
        if ( !isset( $c[$connKey] ) )
        {
            $c[$connKey] = NULL;
        }
        ...
        $c[$connKey] = $this->_conn = mysql_connect( $this->properties['webhost'], $this->properties['webuser'], $this->properties['webpassword'] );
        if ( $this->_conn == NULL )
        {
            return FALSE;
        }
        if ( mysql_select_db( $this->properties['webdatabase'] ) == NULL )
        {
            return FALSE;
        }
        return TRUE;
    ...
    public function _executeQuery( $sqlStatement, $sqlParams, $executionType )
    {
        ...
        if ( $sqlParams != NULL && is_array( $sqlParams ) )
        {
            ...
            {
                $safe_params[] = mysql_real_escape_string( str_replace(array('`', "'"), '', $paramValue), $this->_conn );
            }
