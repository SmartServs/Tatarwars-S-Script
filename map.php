...
if ( intval($_POST['mxp']) < 401 && intval($_POST['mxp']) >'-401' )
    ...
    function getCssClassName2($index)
        {
        return $this->getCssClassAtt($this->matrixSet[$index]);
        }


    function getCssClassAtt($mapItem)
        {
$vid = $mapItem['vid'];
$t11 = new GlobalModel();
$war = $t11->getattackmap($vid,$this->player->playerId,14);
$war1 = $t11->getattackmap($vid,$this->player->playerId,12);
$war2 = $t11->getattackmap($vid,$this->player->playerId,15);
$wars = $t11->getattackmap2($vid,$this->player->playerId,14);

if ($vid == $wars['to_village_id']) {
if(0 < $war)
{
$s = '<span class="m4"></span>';
} else if (0 < $war1) {
$s = '<span class="m5"></span>';
}else if (0 < $war2){
$s = '<span class="m6"></span>';
}
}
        return $s;
        }
