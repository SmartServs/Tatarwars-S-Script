...
$this->errorTable = "أنت لا زالت تحت الحمايه";
return null;
}
...
{
$q = new QueueModel();
$this->efect = $q->provider->fetchScalar( "SELECT artefacts FROM p_villages WHERE is_artefacts='1' and artefacts = '5' and id='".$villageRow['id']."'");
if ($this->efect == 5) {
$this->h = 2;
}else {
...
$q = new QueueModel();
$artefact= 1;
$this->efect = $q->provider->fetchScalar( "SELECT artefacts FROM p_villages WHERE is_artefacts='1' and artefacts = '1' and id='".$this->data['selected_village_id']."'");
if ($this->efect == 1) {
$artefact= 3;
}
...
return ($minSpeed * $artefact) * ($factor / 100);
}
...
$neededCpValue = intval (($this->gameMetadata['cp_for_new_village'] / $GameMetadata['game_speed'])*$this->data['villages_count']);
}
$totalCpRate = 0;
$totalCpValue = 0;
$m = new BuildModel ();
