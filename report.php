...
else if ( $tid == 9 || $tid == 19 || $tid == 29 || $tid == 108 || $tid == 59|| $tid == 69|| $tid == 49|| $tid == 79 )
{
$kingTroopId = $tid;
}
                                if ( $tid == 0 - 1 )
       ...
       $this->reportData['all_attackTroops_dead'] = $totalAttackTroops_live <= $totalAttackTroops_dead;
                            ...
                            {            
                                $wstr = "";
                             if ($artefactResult == "+") {                   
                            $wstr = "تم الاستيلاء على القطعه ألاثريه";
                                    $wstr = "<img src=\"".$GLOBALS['AppConfig']['system']['linksite']."x.gif\" class=\"unit uhero\" align=\"center\" /> ".$wstr;

                            }else if ($artefactResult == "-1"){
                            $wstr = "لم يتم الاستيلاء على القطعة الاثرية,مستوى الخزنة غير كافي";
                                    $wstr = "<img src=\"".$GLOBALS['AppConfig']['system']['linksite']."x.gif\" class=\"unit uhero\" align=\"center\" /> ".$wstr;
                            }else if ($artefactResult == "-2"){
                            $wstr = "لم يتم الاستيلاء على القطعة الاثرية,الخزنه لم تدمر";
                                    $wstr = "<img src=\"".$GLOBALS['AppConfig']['system']['linksite']."x.gif\" class=\"unit uhero\" align=\"center\" /> ".$wstr;
                            }else if ($artefactResult == "-3"){
                            $wstr = "لم يتم الاستيلاء على القطعة الاثرية,الخزنه ممتلئه";
                                    $wstr = "<img src=\"".$GLOBALS['AppConfig']['system']['linksite']."x.gif\" class=\"unit uhero\" align=\"center\" /> ".$wstr;
                            }
                            ...
                            if ( $captureResult != "" )
                            {
                                ...
                                if ( $captureResult == "#" )
                                {
                                    $wstr = "لم يتم تدمير القصر او السكن";
                                }
                                else
                                if ( $captureResult == "+" )
                                {
                                ...
                                if ( $wstr != "" )
                                {
                                    $wstr = "<img src=\"".$GLOBALS['AppConfig']['system']['linksite']."x.gif\" class=\"unit u".$kingTroopId."\" align=\"center\" /> ".$wstr;
                                }
                            ...
                            if ( $wallDestructionResult != "" )
                            {
                                ...
                                if ( $wstr != "" )
                                {
                                    $wstr = "<img src=\"".$GLOBALS['AppConfig']['system']['linksite']."x.gif\" class=\"unit u".$attackWallDestrTroopId."\" align=\"center\" /> ".$wstr;
                                }
                            ...
                            if ( $catapultResult != "" )
                            {
                                ...
                                if ( $catapultResult == "+" )
                                {
                                    $bdestArr[] = "<img src=\"".$GLOBALS['AppConfig']['system']['linksite']."x.gif\" class=\"unit u".$attackCatapultTroopId."\" align=\"center\" /> ".report_p_totallydestr;
                                }
                                else
                                {
                                ...
                                if ( $sss == "+" ) {
                                    $bdestArr[] = "<img src=\"".$GLOBALS['AppConfig']['system']['linksite']."x.gif\" class=\"unit u".$attackCatapultTroopId."\" align=\"center\" /> ".report_p_totallydestr;
                                            ...
                                            if (!isset($toLevel)){
                                            $bdestArr[] = "<img src=\"".$GLOBALS['AppConfig']['system']['linksite']."x.gif\" class=\"unit u".$attackCatapultTroopId."\" align=\"center\" /> ".report_p_notdestr." ".constant( "item_".$itemId );
                                        }else
                                            if ( $toLevel == 0 - 1 )
                                        {
                                            $bdestArr[] = "<img src=\"".$GLOBALS['AppConfig']['system']['linksite']."x.gif\" class=\"unit u".$attackCatapultTroopId."\" align=\"center\" /> ".report_p_notdestr." ".constant( "item_".$itemId );
                                        ...
                                        {
                                            $bdestArr[] = "<img src=\"".$GLOBALS['AppConfig']['system']['linksite']."x.gif\" class=\"unit u".$attackCatapultTroopId."\" align=\"center\" /> ".report_p_wasdestr." ".constant( "item_".$itemId );
                                        }
                                        else if ($toLevel)
                                        {
                                            $bdestArr[] = "<img src=\"".$GLOBALS['AppConfig']['system']['linksite']."x.gif\" class=\"unit u".$attackCatapultTroopId."\" align=\"center\" /> ".report_p_waslowered." ".constant( "item_".$itemId )." ".report_p_fromlevel." ".$fromLevel." ".report_p_to." ".$toLevel;
                                        }
                                    }
                                }
                        ...
                        case 4 :
                            ...
                            if ( $spyType == 3 )
                            {
                                ...
                                $this->reportData['harvest_info'] = "لم ينجُ أحد من جواسيسك";
                            }
                            $attackTroopsStrArr = explode( ",", $attackTroopsStr );
                    ...
                    {
                        $canOpenReport = TRUE;
                        $var = ($_GET['id']*$_GET['id']/100);
                        $hash = base64_encode(base64_encode($var));
 
                        if ( ($this->player->playerId != $from_player_id && $this->player->playerId != $to_player_id) OR ($_GET['hash'] == $hash))
                        {
             $this->showList=0;
                        ...
                        if ($_GET['hash'] == $hash) {$canOpenReport = 1; $this->showList = 0;}
                        if ( $canOpenReport )
                        {
$this->showList = 0;
